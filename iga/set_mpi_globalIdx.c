void set_mpi_globalIdx(int *globalIdx, int *iDof_displ_array, int nselfIdx, int *selfIdx, int nsendPart, int *sendPart, int nsendIdx, int *sendIdx, int *sendPtr, int nrecvPart, int *recvPart, int nrecvIdx, int *recvIdx, int *recvPtr, int nbasis[], int ndof, int nproc, int rank)
{
    ...
    for (int isendPart=0; isendPart<nsendPart; isendPart++) { ierror=MPI_Send(globalIdx_send+sendPtr[isendPart],sendPtr[isendPart+1]-sendPtr[isendPart],MPI_INT,sendPart[isendPart],1,MPI_COMM_WORLD);assert(ierror==MPI_SUCCESS); }
    for (int irecvPart=0; irecvPart<nrecvPart; irecvPart++) { ierror=MPI_Recv(globalIdx_recv+recvPtr[irecvPart],recvPtr[irecvPart+1]-recvPtr[irecvPart],MPI_INT,recvPart[irecvPart],1,MPI_COMM_WORLD,&status);assert(ierror==MPI_SUCCESS); }
